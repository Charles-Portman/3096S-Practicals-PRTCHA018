...
int main(void)
{
  ...
  for (int k=0;k<6;k++){
	  write_to_address(0b0+k*8,patterns[k]);
  }
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  if (HAL_GPIO_ReadPin(GPIOA,Button0_Pin) == 0b1){
		  if (timer_value == 1){
			  timer_value = 0;
			  TIM16 -> PSC = 0b1111100111111;
		  } else {
			  timer_value = 1;
			  TIM16 -> PSC = 0b111110100000;
		  }
	  }
  }
  /* USER CODE END 3 */
...
void TIM16_IRQHandler(void)
{
	...
	if (read_from_address(read_value*8) == patterns[read_value]){
		//HAL_GPIO_WritePin(GPIOB,GPIO_PIN_All,read_from_address(read_value*8));
	  HAL_GPIO_WritePin(GPIOB,LED0_Pin,(read_from_address(read_value*8)>>7)&0b1);
	  HAL_GPIO_WritePin(GPIOB,LED1_Pin,(read_from_address(read_value*8)>>6)&0b1);
	  HAL_GPIO_WritePin(GPIOB,LED2_Pin,(read_from_address(read_value*8)>>5)&0b1);
	  HAL_GPIO_WritePin(GPIOB,LED3_Pin,(read_from_address(read_value*8)>>4)&0b1);

	  HAL_GPIO_WritePin(GPIOB,LED4_Pin,(read_from_address(read_value*8)>>3)&0b1);
	  HAL_GPIO_WritePin(GPIOB,LED5_Pin,(read_from_address(read_value*8)>>2)&0b1);
	  HAL_GPIO_WritePin(GPIOB,LED6_Pin,(read_from_address(read_value*8)>>1)&0b1);
	  HAL_GPIO_WritePin(GPIOB,LED7_Pin,(read_from_address(read_value*8)>>0)&0b1);
	}
	 else {
		HAL_GPIO_WritePin(GPIOB,LED0_Pin,0b1);
	 }
