...
#define TIM2CLK 8000	// STM Clock frequency
...
uint32_t triangle_LUT[NS] = {528,544,560,576,592,608,624,640,656,672,688,704,720,736,752,768,783,799,815,831,847,863,879,895,911,927,943,959,975,991,1007,1023,1007,991,975,959,943,927,911,895,879,863,847,831,815,799,783,768,752,736,720,704,688,672,656,640,624,608,592,576,560,544,528,512,496,480,464,448,432,416,400,384,368,352,336,320,304,288,272,257,241,225,209,193,177,161,145,129,113,97,81,65,49,33,17,1,16,32,48,64,80,96,112,128,144,160,176,192,208,224,240,256,271,287,303,319,335,351,367,383,399,415,431,447,463,479,495,511};
...
int main(void)
{
  ...
  HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_1, Sin_LUT, NS);
...
void EXTI0_1_IRQHandler(void)
{
	...
	uint32_t Debdel = HAL_GetTick();
	for (int i = Debdel; i < (Debdel + 10); i++) {
		int k = 0;
	}
	...
	switch(posi){
	case 0:
		SRCAddress =  (uint32_t)& (saw_LUT);
		DestAddress = (uint32_t) &(TIM3->CCR1);
		HAL_DMA_Start_IT(&hdma_tim2_ch1,SRCAddress, DestAddress, NS);
		posi = 1;
	break;
	case 1:
		SRCAddress =  (uint32_t)& (triangle_LUT);
		DestAddress = (uint32_t) &(TIM3->CCR1);
		HAL_DMA_Start_IT(&hdma_tim2_ch1,SRCAddress, DestAddress, NS);
		posi = 2;
	break;
	case 2:
		SRCAddress =  (uint32_t)& (Sin_LUT);
		DestAddress = (uint32_t) &(TIM3->CCR1);
		HAL_DMA_Start_IT(&hdma_tim2_ch1,SRCAddress, DestAddress, NS);
		posi = 3;
	break;
